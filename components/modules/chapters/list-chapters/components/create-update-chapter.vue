      ...
      <v-card-title>
        <v-row>
          <v-col
            ...
            sm="6"
            md="6">
            <h4>{{ chapterId ? 'UPDATE' : 'CREATE'}} CHAPTER</h4>
          </v-col>
          <v-col
            cols="12"
            sm="6"
...
export default {
  ...
  fetch () {
    if (this.chapterId) this.fetchMembership()
  },

  methods: {
    fetchMembership () {
      this.loading = true
      this.API_POST({ url: 'Chapters/fetchById/' + this.chapterId})
        .then(response => {
    ...
    confirmSubmit () {
      ...
      let type = this.chapterId ? 'Update' : 'Create'
      this.confirm.show = true
      this.confirm.msg = `Are you sure you want to ${type.toLowerCase()} this Chapter?`
      this.confirm.title = `${type} Chapter`
    },
    ...
    submit () {
      ...
      this.API_POST({url: `Chapters/${method}`, data: formData})
        .then(response => {
          this.resetForm()
          this.$emit('event')
